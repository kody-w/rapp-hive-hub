# The Product Launch Company: starter

Launch a real offline agenda planner without inventing traction.

Coordinate product strategy, design, engineering, creative, distribution, and customer success
around Agenda Pocket, an original usable local demo with explicit acceptance and approval-gated
launch work.
...
2. **Organization** (specified): the org card `product-launch-company` plans one accountable owner,
   one world (planned id `demo-product-launch-company`), one policy, one release scope and exactly
...
- with `npx degit kody-w/rapp-hive-hub/starters/product-launch-company#experimental/organism-fit product-launch-company`;
- or as a ZIP of the repository.

## Create the Hive

1. Ask your Brainstem to create a Hive with its Hive agent: `create` with the title "The Product
   Launch Company", `approvals` 2 and the `fields` hint from `HIVE.md`. The Hive agent writes a
...
- `shared/product-strategy/`: Product strategy. Own the problem hypothesis, scope, measurable
  acceptance, and conditional launch decision. 2 tasks.
- `shared/design/`: Interaction and accessibility design. Make time-budget tradeoffs understandable
  and test keyboard, reading order, and error recovery. 2 tasks.
- `shared/engineering/`: Offline product engineering. Maintain the runnable demo, deterministic
  agenda core, and reproducible release evidence. 2 tasks.
- `shared/creative/`: Product storytelling. Prepare accurate copy and a clearly labeled storyboard
  without pretending media was generated. 2 tasks.
- `shared/distribution/`: Launch operations. Plan approved channels, reversible publication, and an
  evidence-linked launch runbook. 2 tasks.
- `shared/customer-success/`: Support and learning. Prepare practical help, a local-data boundary,
  and a consent-aware feedback instrument. 2 tasks.
- `shared/casework/`: the charter (`product-launch-company.md`), the first case
  (`agenda-pocket-first-launch.md`: Agenda Pocket: make a 60-minute meeting fit without hiding
  required topics) and 15 starter files in `artifacts/`.

## What maps cleanly, and what does not

- 7 of 15 starter files are not markdown, so they sit in `shared/casework/artifacts/` as fenced
...
- The organization's planned world id (`demo-product-launch-company`) has no folder-Hive
