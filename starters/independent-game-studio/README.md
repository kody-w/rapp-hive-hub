# The Independent Game Studio: starter

Ship a small original game, with a real production discipline.

Run a seven-team independent studio around Mosslight Courier, an original offline route puzzle. Turn
synthetic first-session feedback into a bounded onboarding revision with playable source, readable
art, opt-in sound, and reproducible regression evidence.
...
2. **Organization** (specified): the org card `independent-game-studio` plans one accountable owner,
   one world (planned id `demo-independent-game-studio`), one policy, one release scope and exactly
...
- with `npx degit kody-w/rapp-hive-hub/starters/independent-game-studio#experimental/organism-fit independent-game-studio`;
- or as a ZIP of the repository.

## Create the Hive

1. Ask your Brainstem to create a Hive with its Hive agent: `create` with the title "The Independent
   Game Studio", `approvals` 2 and the `fields` hint from `HIVE.md`. The Hive agent writes a fresh
...
- `shared/production/`: producer. Own the first-shift scope, review gates, and a conditional release
  decision without claiming a shipped product. 2 tasks.
- `shared/design/`: game-designer. Specify understandable bridge, carrying, delivery, and undo rules
  within the existing three-board game. 1 task.
- `shared/engineering/`: game-engineer. Maintain the deterministic engine and keyboard-first offline
  browser build. 1 task.
- `shared/art/`: game-artist. Create original high-contrast symbols and a small coherent visual
  asset system. 1 task.
- `shared/audio/`: sound-designer. Specify restrained, opt-in, locally synthesized event cues with a
  silent fallback. 1 task.
- `shared/quality/`: qa-lead. Check solvability, input handling, no-network behavior, accessibility
  limits, and regressions. 2 tasks.
- `shared/player-research/`: player-researcher. Separate the synthetic feedback exercise from real
  observations and design the next consent-based playtest. 2 tasks.
- `shared/casework/`: the charter (`independent-game-studio.md`), the first case
  (`first-shift-onboarding.md`: Mosslight Courier: make the first bridge legible) and 13 starter
  files in `artifacts/`.

## What maps cleanly, and what does not

- 10 of 13 starter files are not markdown, so they sit in `shared/casework/artifacts/` as fenced
...
- The organization's planned world id (`demo-independent-game-studio`) has no folder-Hive
