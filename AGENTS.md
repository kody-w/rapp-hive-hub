...
- Seed packages only gain files. A changed package gets a successor record;
  keep its predecessor byte-exact at its content addresses, outside the active
  dialbook, with `scripts/seed_predecessors.py`.
- Folder-Hive templates and `ORGANISM.md` are inert data generated by
  `scripts/folder_hive.py`; never hand-edit them.
